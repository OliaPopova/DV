            ...
            dbc.Row([
                ...
                dbc.Card([
                    dbc.Container([
                        dbc.Row([
                            html.P(
                                "Объём СГЗ НИОКР", id='text5',
                                className="card-text",
                                style={'font-size': '16px',
                                       'font-family': 'Open Sans'}),
                            dcc.Textarea(id='textarea5P5', className="textarea", readOnly=True,
                                         style={}),
...
def update_figure(selected_F5,selected_P2, selected_F2, selected_S1, selected_Pr1):
    ...
    fig = px.bar(data_frame=df, x='год', y='значение', text_auto='.4s', template='plotly', title='P5_c')
    fig.update_layout(
        plot_bgcolor='#515151',
...
@app.callback(Output('textarea5P5', 'value'), [Input('F5', 'value')])
